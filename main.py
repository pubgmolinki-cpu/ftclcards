...
async def process_buy(callback: types.CallbackQuery):
    ...
    try:
        ...
        if action == "vip":
            ...
            conn.commit()
            await callback.message.answer(f"🚀 VIP активирован до {expire.strftime('%H:%M')} завтрашнего дня!")
        else:
...
async def profile(message: types.Message):
    ...
    try:
        ...
        if u and u['vip_until'] and u['vip_until'] > datetime.now():
            vip_status = f"✅ до {u['vip_until'].strftime('%d.%m %H:%M')}"
