...
async def get_not_subscribed_channels(uid):
    not_subscribed = []
    channels = {"@ftclcardschannel": "https://t.me/ftclcardschannel", "@waxteamiftl": "https://t.me/waxteamiftl", "@ftcloff": "https://t.me/ftcloff"}
    for ch_name, ch_url in channels.items():
        try:
            m = await bot.get_chat_member(ch_name, uid)
            if m.status not in ["member", "administrator", "creator"]: not_subscribed.append((ch_name, ch_url))
        except: not_subscribed.append((ch_name, ch_url))
    return not_subscribed

# --- АДМИНКА (ДОБАВЛЕНИЕ КАРТЫ) ---
...
async def add_player(message: types.Message, command: CommandObject):
    ...
    try:
        ...
        await message.answer(f"✅ Карта <b>{name}</b> ({rating}) добавлена! Редкость: {RARITY_MAP[rarity_key]['name']}")
...
async def process_buy(call: types.CallbackQuery):
    ...
    if cur.fetchone()['balance'] < prices[item]: return await call.answer("❌ Нет звёзд!", show_alert=True)
    ...
    if item == "vip":
        ...
        await call.message.answer("💎 VIP активирован!")
    else:
        q = {"bronze": "50 AND 70", "gold": "75 AND 85", "brilliant": "= 90"}[item]
...
async def games(message: types.Message):
    kb = InlineKeyboardBuilder().button(text="🧩 Угадай игрока", callback_data="game_guess").as_markup()
    await message.answer("🎮 Игры:", reply_markup=kb)

@dp.callback_query(F.data == "game_guess")
async def start_guess(call: types.CallbackQuery):
    uid = call.from_user.id
    if time.time() - user_cooldowns.get(uid, {}).get("guess", 0) < 18000:
        rem = int(18000 - (time.time() - user_cooldowns.get(uid, {}).get("guess", 0)))
        return await call.answer(f"⏳ Жди {rem//3600}ч.", show_alert=True)
    waiting_for_bet[uid] = "guess"; await call.message.edit_text("💰 Ставка (макс 30000):")
...
async def bet_handler(message: types.Message):
    ...
    if bet > 30000: return await message.answer("❌ Больше 30000 нельзя!")
    ...
    user_bets[uid] = {"bet": bet, "corr": correct['id']}
    kb = InlineKeyboardBuilder()
...
async def ans_handler(call: types.CallbackQuery):
    ...
    conn.commit(); cur.close(); conn.close()
    await call.message.edit_text(f"{'✅ +'+str(data['bet']) if win else '❌ -'+str(data['bet'])} ⭐")
...
async def collection(call: types.CallbackQuery):
    ...
    txt = f"💼 Коллекция ({page+1}):\n" + "\n".join([f"▪️ {r[0]} ({r[1]})" for r in res])
    kb = InlineKeyboardBuilder()
    if page > 0: kb.button(text="⬅️", callback_data=f"vcoll_{page-1}")
    if total > (page+1)*15: kb.button(text="➡️", callback_data=f"vcoll_{page+1}")
...
async def profile(message: types.Message):
    ...
    await message.answer(f"👤 @{u['username']}\n💰 {u['balance']:,} ⭐\n💎 VIP: {'✅' if is_v else '❌'}", reply_markup=InlineKeyboardBuilder().button(text="💼 Коллекция", callback_data="vcoll_0").as_markup())
    cur.close(); conn.close()

@dp.message(F.text == "Рефералка 👥")
async def ref(message: types.Message):
    me = await bot.get_me(); await message.answer(f"Твоя ссылка (5000 ⭐ за друга):\n<code>t.me/{me.username}?start={message.from_user.id}</code>")
...
async def check_s(call: types.CallbackQuery):
    if not await get_not_subscribed_channels(call.from_user.id): await call.message.delete(); await call.message.answer("✅ Подписан! Жми 'Получить Карту'")
