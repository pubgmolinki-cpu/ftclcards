...
def get_stars_by_rating(rating):
    if rating >= 99: return 10000 # Награда за Legend
    if rating >= 95: return 5000  # Награда за Ivents
...
def get_card(rarity_filter=None):
    ...
    try:
        if rarity_filter:
            ...
            # ДРОБНЫЕ ШАНСЫ (через диапазон 1000)
            r = random.randint(1, 1000)
            if r == 1: 
                rtype = "legend"      # 0.1% (шанс 1 из 1000)
            elif 2 <= r <= 6: 
                rtype = "ivents"      # 0.5% (шанс 5 из 1000)
            elif 7 <= r <= 56: 
                rtype = "brilliant"   # 5%
            elif 57 <= r <= 206: 
                rtype = "gold"        # 15%
        ...
        card = cur.fetchone()
        if not card: # Если в категории пусто, берем любую
...
async def open_shop(message: types.Message):
    ...
    # ЦЕНЫ УВЕЛИЧЕНЫ В 2 РАЗА ДЛЯ БАЛАНСА
...
async def profile(message: types.Message):
    ...
    try:
        ...
        kb = InlineKeyboardBuilder(); kb.button(text="Посмотреть Коллекцию 🗂", callback_data="show_collection")
...
async def add_player(message: types.Message):
    ...
    except: await message.answer("❌ Ошибка формата! Имя, Рейтинг, Поз, Клуб")
