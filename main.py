...
dp = Dispatcher()
cooldowns = {} 
temp_photo_buffer = {} # Буфер для хранения photo_id перед добавлением игрока
...
async def cmd_start(message: types.Message, command: CommandObject):
    ...
    if is_new and referrer_id and referrer_id.isdigit() and int(referrer_id) != user_id:
        ...
        try: await bot.send_message(ref_id, f"🎁 У вас новый реферал! Зачислено <b>5000 ⭐</b>")
        except: pass
    ...
    kb.adjust(2, 2, 1)
    await message.answer("⚽️ <b>FTCL Cards приветствует тебя! Собирай карточки лучших игроков лиги ФТКЛ!</b>", reply_markup=kb.as_markup(resize_keyboard=True))
...
async def give_card_free(message: types.Message):
    ...
    if user_id in cooldowns and now - cooldowns[user_id] < 43200: 
        rem = int(43200 - (now - cooldowns[user_id]))
...
async def view_collection(callback: types.CallbackQuery):
    ...
    if not cards: return await callback.answer("Ваша коллекция пока пуста!", show_alert=True)
    text = "🗂 <b>Твоя коллекция:</b>\n\n"
    for c in cards: text += f"▫️ {html.escape(c['name'])} ({c['rating']})\n"
...
async def buy_pack(callback: types.CallbackQuery):
    ...
    conn.commit(); cur.close(); conn.close()
    await callback.message.reply_photo(photo=card['photo_id'], caption=f"👤 <b>{html.escape(card['name'])}</b>\n💰 Награда: <b>+{reward}</b> ⭐")
...
async def handle_photo(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        await message.reply("📸 Фото получено и сохранено в буфер. Теперь отправь:\n\n<code>/add_player Имя, Рейтинг, Позиция, Клуб</code>")

@dp.message(Command("add_player"))
async def add_player(message: types.Message):
    if message.from_user.id != ADMIN_ID: return
    p_id = temp_photo_buffer.get(ADMIN_ID)
    if not p_id: return await message.reply("❌ Сначала отправь фото карточки!")
    
    try:
        # Разделяем аргументы через запятую
        ...
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute("""INSERT INTO all_cards (name, rating, position, rarity, rarity_type, club, photo_id) 
                       VALUES (%s,%s,%s,%s,%s,%s,%s)""", 
                    (name, rating, pos, rlabel, rtype, club, p_id))
        conn.commit(); cur.close(); conn.close()
        
        await message.answer(f"✅ Игрок <b>{name}</b> ({rating}) успешно добавлен в базу!")
        del temp_photo_buffer[ADMIN_ID] # Очищаем буфер после успеха
    except Exception as e:
        await message.answer(f"❌ Ошибка формата или данных.\nПример: <code>/add_player L. Messi, 91, RW, Inter Miami</code>")
...
async def admin_reset(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        conn.commit(); cur.close(); conn.close()
        await message.answer("✅ Весь прогресс игроков сброшен.")
